...
def read_data():
    ...
    infile = open("ner_dataset.csv", "r") #for task 2 I'm considering "ner_dataset.csv", for task 3 I'm considering "my_dataset.csv"
    #infile = open("my_dataset.csv", "r") 
    #for task 3, I'm considering "my_dataset.csv", if uncommented this line and commented above line we can achive task 3. For the easy executing, I'm attaching a separate Task3_A2.py.
...
def tag_sentences(sent, m, wordmap, reverse_tagmap, maxlen):
    ...
    predictions = m.predict(v_sent)  # use the model to predict tags
...
def give_entities(sentence, tags):
    """ Given a sentence and BIO tags, gives a list of entities
...
def main():
    ...
    # sent, pos, ner = read_data("ner_dataset.csv", "own_dataset.csv")
    ...
    tag_len = len(tagmap)
    entity_types = [p[2:] for p in tagmap if len(p) > 2 and p[:2] == "B-"]
    ...
    # Create RNN with SimpleRNN layer
    m_simple_rnn = Sequential()
    m_simple_rnn.add(Embedding(vocab_len, embed_len, embeddings_initializer=Constant(embedding_matrix)))
    m_simple_rnn.add(Bidirectional(SimpleRNN(10, return_sequences=True, activation="relu")))
    m_simple_rnn.add(TimeDistributed(Dense(50, activation="relu")))
    m_simple_rnn.add(Dense(tag_len, activation="softmax"))
    m_simple_rnn.compile("adam", "categorical_crossentropy", metrics=["accuracy"])

    # training
    m_simple_rnn.fit(v_tr_sent, v_tr_target, batch_size=100, epochs=2, verbose=1)

    # testing
    te_predicted_simple_rnn = tag_sentences(te_sent, m_simple_rnn, wordmap, reverse_tagmap, maxlen)

    # evaluation
    evaluate_ie(te_sent, te_tag, te_predicted_simple_rnn, entity_types, "eval_simple_rnn.txt")

    # Create RNN with LSTM layer
    m_lstm = Sequential()
    m_lstm.add(Embedding(vocab_len, embed_len, embeddings_initializer=Constant(embedding_matrix)))
    m_lstm.add(Bidirectional(LSTM(10, return_sequences=True, activation="relu")))
    m_lstm.add(TimeDistributed(Dense(50, activation="relu")))
    m_lstm.add(Dense(tag_len, activation="softmax"))
    m_lstm.compile("adam", "categorical_crossentropy", metrics=["accuracy"])

    # training
    m_lstm.fit(v_tr_sent, v_tr_target, batch_size=100, epochs=2, verbose=1)

    # testing
    te_predicted_lstm = tag_sentences(te_sent, m_lstm, wordmap, reverse_tagmap, maxlen)

    # evaluation
    evaluate_ie(te_sent, te_tag, te_predicted_lstm, entity_types, "eval_lstm.txt")
